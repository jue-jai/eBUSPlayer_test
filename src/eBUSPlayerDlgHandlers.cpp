...
// =============================================================================
void eBUSPlayerDlg::OnCbnDropdownMode()
{
    if ( !mPlayer->IsDeviceConnected() )
    {
        return;
    }
    OnUpdateAcquisitionModes( NULL, NULL );
}
...
LRESULT eBUSPlayerDlg::OnUpdateSource( WPARAM wParam, LPARAM lParam )
{
    ...
    if ( lResult.IsOK() )
    {
        ...
        SendMessage( WM_UPDATEACQUISITIONMODES );
...
// =============================================================================
LRESULT eBUSPlayerDlg::OnReopenStreamIfNeeded( WPARAM wParam, LPARAM lParam )
{
    PVUNREFPARAM( wParam );
    PVUNREFPARAM( lParam );
    ...
    {
        // Stream parameters would typically be re-created, close stream window
        CloseWnd( &mStreamParametersWnd );

        // Updating the source will at some point correctly re-open the stream for current needs
        SendMessage( WM_UPDATESOURCE );
...
// =============================================================================
LRESULT eBUSPlayerDlg::OnDisconnect( WPARAM wParam, LPARAM lParam )
{
    PVUNREFPARAM( wParam );
    PVUNREFPARAM( lParam );
