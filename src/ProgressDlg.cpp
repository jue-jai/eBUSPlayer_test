...
BEGIN_MESSAGE_MAP(ProgressDlg, CDialog)
    ON_WM_DESTROY()
    ON_WM_TIMER()
    ON_BN_CLICKED(IDC_CANCELTASK, &ProgressDlg::OnCancelButtonClicked)
END_MESSAGE_MAP()
...
void ProgressDlg::DoDataExchange( CDataExchange* pDX )
{
    ...
    DDX_Control( pDX, IDC_CANCELTASK, mCancelTaskButton );
}
...
///
/// \brief Makes the cancel button usable (or not)
///

void ProgressDlg::SetCancellable( bool aCancellable )
{
    mCancellable = aCancellable;    
    if ( mCancelTaskButton.GetSafeHwnd() != 0 )
    {
        mCancelTaskButton.EnableWindow( ( aCancellable ) ? TRUE : FALSE );
        mCancelTaskButton.ShowWindow( ( aCancellable ) ? SW_SHOW : SW_HIDE );
    }
}
...
void ProgressDlg::OnCancelButtonClicked()
{
    mCancelled = true;
}
