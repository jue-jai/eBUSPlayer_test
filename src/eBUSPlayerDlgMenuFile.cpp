...
void eBUSPlayerDlg::InitFileMenu( CMenu* pMenuItem, UINT aID, int aIndex )
{
    ...
    case ID_FILE_SAVE:
        pMenuItem->EnableMenuItem( aIndex, MF_BYPOSITION | 
            ( ( ( mPlayer->IsDeviceConnected() || mPlayer->IsStreamOpened() ) && !mPlayer->IsStreaming() ) ? MF_ENABLED : MF_GRAYED ) );
        break;
    ...
    case ID_FILE_LOAD:
        pMenuItem->EnableMenuItem( aIndex, MF_BYPOSITION | 
            ...
            !mPlayer->IsStreaming() ) ? MF_ENABLED : MF_GRAYED ) );
        break;
...
void eBUSPlayerDlg::SaveMRUToRegistry()
{
    ...
    while( lIt != mRecentList.end() ) 
    {
        ...
        pApp->WriteProfileString(_T("MRU"), lNumber, lToRegistry);
        lIt++;
        lCount++;
    }
